...
!----------------------------------------------------------------------
 ...
 contains
   subroutine makepos(n_)
     ...
     integer :: i1
     do i1 = 1, n_                                 !     To help visualise - each row follows on from the previous. The tilde quantities take twice as ,uch space as they have real and imag comps.
...
!      inv_pos(          i1 + 2, :) = (i1,     NBAR, RL) ! This ended up not being used but might be useful somehow
 ...
 subroutine newtonhook(f, df, dfp, sub, dp, m, n, gtol, &
   ...
   do while(.true.)
      ...
      print*, 'max_gits = ', new_gits, 'gdel= ', gdel
